...
Date: 2026-08-31 · original analysis baseline; Phase 1 implementation evidence added 2026-09-01.
**Rev 2** — corrected against Bert's review the same day. Correction log in §0.

> **2026-09-01 Phase 1 implementation update:** the original analysis below is retained as the
> pre-change baseline. `WaterRuntimeRelevance` now builds one allocation-free-after-warm-up,
> deterministic per-camera snapshot. Its order is importance pin, selected submerged/fog body,
> frustum visibility, projected bounds coverage, nearest point of `CullBounds`, then registration
> order. Simulation, planar reflection, caustic projection and after-fog foam collection consume
> that snapshot. The simulation center-distance defect is corrected; planar remains capped at three;
> screen-space caustics are capped by a new resolved-tier budget (default four); foam overlay
> collection is frustum filtered. The caustic relevance test uses the full volumetric `CullBounds`,
> not only the surface sheet, so off-screen water whose underwater projection column can intersect
> the camera remains eligible; only a complete water column outside the frustum is proven irrelevant.
> The unbounded-ocean FFT path, topology/query system, exclusion registry, per-body terrain clipping,
> simulation resolution and lease lifetime are unchanged. See
> `AUDIT_multi_body_defaults_2026-09-01.md` for ownership and unresolved default conflicts.

### Phase 1 measured profile (2026-09-01)

Unity Profiler recorders sampled the saved `Assets/connected waters.unity` scene after 90 warm-up
frames for 240 measured frames in Unity 6000.3.9f1, Direct3D11, on an NVIDIA GeForce RTX 5060
Laptop GPU. The saved scene contains four `WaterVolume` bodies and three river renderers (the newer
builder recipe creates five bodies, so these are not a five-body-builder measurement).

| Measurement | Measured value |
|---|---:|
| Batch-Editor frame rate | 146.41 FPS |
| CPU frame time (`FrameTimingManager`, 240/240 samples) | 6.834 ms |
| `RuntimeRelevance.Build` | 0.0303 ms/frame, 1 call/frame |
| Body renderer publication | 0.1179 ms/frame, 4 calls/frame |
| Body global publication | 0.0184 ms/frame, 1 call/frame |
| River renderer publication | 0.0967 ms/frame, 3 calls/frame |

No GPU frame-timing samples were returned by the batch Editor, so **GPU time is UNMEASURED**, not
zero. The caustic-projection and foam-overlay markers had no samples in this saved scene/run; that
proves only that those passes did not execute in this capture. Batch-Editor FPS is evidence for this
machine and scene only, not a shipping-player target or a 1/5/15/30-body scaling curve.

### Repeatable 1/5/15/30 stress scene (2026-09-01)

`Assets/Multi-Body Water Stress Test.unity` is generated by
`Editor/WaterBuildKit.MultiBodyStressTest.cs`. It contains 30 bodies in cumulative roots, one
primary, four connected river ribbons, five simple stepped concrete basins and geometric river
banks. The first five bodies request planar reflections and all 30 request projected caustics, so
the shared planar/caustic grants are exercised instead of producing a simulation-only benchmark.
The other 25 bodies intentionally have no decorative basin geometry, particle foam, splash or god
rays; this keeps prop cost from hiding water-count scaling.

In Play Mode, keys `1`, `2`, `3`, `4` select exactly 1, 5, 15 or 30 active bodies. `Space` pauses the
camera and `R` restarts its 24-second deterministic route. For comparable captures, select a tier,
press `R`, record one complete route, then repeat without changing quality, resolution, window size
or profiler settings. The scene-generation menu is
`Tools > Abstract Occlusion > Water > Create Multi-Body Stress Test Scene`; the GameObject menu can
also rebuild the rig in the current scene.
