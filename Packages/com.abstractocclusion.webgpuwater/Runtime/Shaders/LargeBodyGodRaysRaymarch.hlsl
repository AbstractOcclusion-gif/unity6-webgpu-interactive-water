// WebGpuWater - ocean god-ray RAYMARCH body, shared by the two raymarch passes of
// LargeBodyGodRays.shader (moved VERBATIM out of pass 0 on 2026-09-02 - any behaviour change
// here is a bug). WHY two passes: `#pragma skip_optimizations webgpu` cannot be scoped to a
// keyword variant, and the translator bug it works around fires only in the WATER_FOG_SIMPLE
// fork (see the guard note on the Simple pass). Splitting the fork into two passes lets the Full
// march ship optimised on web again while the Simple march keeps the guard. Which fork this
// translation unit compiles is decided by the INCLUDING pass: it `#define`s WATER_FOG_SIMPLE
// before this include, or does not - the same macro the keyword used to set, read by the same
// `#ifdef`s, so both forks are byte-identical to their former variants.
//
// Everything below is one pass's HLSLPROGRAM body (declarations, helpers, Vert, FragRaymarch):
// include it from exactly one pass per translation unit, after the pass's own #pragmas.
#ifndef WEBGPUWATER_LARGE_BODY_GOD_RAYS_RAYMARCH_INCLUDED
#define WEBGPUWATER_LARGE_BODY_GOD_RAYS_RAYMARCH_INCLUDED
...
#endif // WEBGPUWATER_LARGE_BODY_GOD_RAYS_RAYMARCH_INCLUDED
